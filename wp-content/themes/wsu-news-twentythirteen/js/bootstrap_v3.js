...
function load_base(url) {
	setTimeout(function(){
		...
		if( loadJquery !="no" && (typeof(jQuery) === 'undefined' || !window.jQuery) ){// || (jQuery().jquery!=ver_jquery || jQuery.fn.jquery!=ver_jquery) ){
			async_load_js('//ajax.googleapis.com/ajax/libs/jquery/'+ver_jquery+'/jquery.min.js');
			pause= pause>0?100:0;
			setTimeout(function(){
				load_base(url);
			...
			(function($) {
				...
				scriptArray = [ // this is where we'd load the scriptArray list dynamicly.  Right now it's hard coded
					{
						src:"/wp-content/themes/wsu-news-twentythirteen/js/jquery.jTrack.min.0.2.1.js",
						exc:function(){
